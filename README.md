...
```
### 🔹Q8.Fill missing Outlet_Size with “Medium”.
```python
...
### 🔹 Q9.Create a new column MRP_Category:“Low” if Item_MRP < 100“Medium” if 100 ≤ Item_MRP < 200“High” if Item_MRP ≥ 200**
```python
...
```
### 🔹 Q10.Create a new column Visibility_Flag:“Low Visibility” if Item_Visibility < 0.05“Normal Visibility” otherwise
```python
df.withColumn('Visibility_Flag',when(col('Item_Visibility')<0.05,'Low').otherwise('Normal')).display()
```
### 🔹 Q11.Standardize Item_Fat_Content (convert “LF”, “low fat”, “Low Fat” → “Low Fat”; “reg”, “Regular” → “Regular”).
```python
...
### 🔹Q17 For each Outlet_Identifier, rank items based on their Item_Outlet_Sales.
```python
...
```
### 🔹Q18 Get the top 3 selling items in each outlet explain
```python
...
```
### 🔹Q19 Filter all records where Item_Visibility > 0.2 and Item_Outlet_Sales < 1000.
```python
df_filter = df.filter((col('Item_Visibility')>0.2)&(col('Item_Outlet_Sales')<1000)).display()
```
### 🔹Q20.Filter outlets located in “Tier 3” cities with total sales above average.
```python
...
### 🔹Q21.Create a pivot table showing average Item_Outlet_Sales by Outlet_Type (rows) and Outlet_Location_Type (columns)
```python
pivot_df = (
